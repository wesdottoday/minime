...
siblings:
  ...
  - "2f"
  - "2g"
url: "/series/inference/what-happens/embeddings/model-layers/"
aliases:
  - "/llms/what-happens/embeddings/model-layers/"
...
A layer is one complete round of transformation that every token vector passes through. If the model has 80 layers (typical for a large LLM), then each token vector gets transformed 80 times in sequence before the model produces its output. During [prefill](/series/inference/what-happens/prefill-decode/) (see [prefill vs decode](/series/inference/what-happens/prefill-decode/)), all tokens in the input — your message, prior conversation history, system prompt, everything — enter each layer simultaneously and are processed in parallel.
...
1. **Raw [embedding](/series/inference/what-happens/embeddings/)** (before any layers): "I am the token 'sat'" — no context, just the static lookup

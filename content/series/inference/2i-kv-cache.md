...
children:
  ...
  - "3k"
siblings: []
url: "/series/inference/what-happens/prefill-decode/kv-cache/"
aliases:
  - "/llms/what-happens/prefill-decode/kv-cache/"
...
graph LR
    ...
    click E "/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/"
    click F "/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/"
    click G "/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/"
    click H "/series/inference/what-happens/embeddings/model-layers/"
    click I "/series/inference/what-happens/embeddings/model-layers/ffn-deep-dive/"
    click J "/series/inference/what-happens/embeddings/model-layers/"
    click L "/series/inference/what-happens/embeddings/model-layers/final-vector-to-token/"
    click M "/series/inference/what-happens/embeddings/model-layers/final-vector-to-token/"
```
...
That's just the cache — not the model weights, not the activations (the transient intermediate values computed and discarded at each step during inference — the expanded vector before [SiLU](/series/inference/what-happens/embeddings/layer-transforms/), the [attention scores](/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/) before softmax, etc.), just the stored keys and values. A B200 has 192 GB of HBM3e. So this single request's KV cache already exceeds the memory of one GPU. This is why long context is fundamentally a **memory capacity problem** before it's a compute problem. You can do tricks to reduce the compute ([sparse attention](/series/inference/what-happens/prefill-decode/kv-cache/sparse-attention/), approximations), but you still need somewhere to *store* all those K and V vectors if the model needs to attend to them.

This is exactly where KV cache management becomes a critical infrastructure concern:
- **KV cache [quantization](/series/inference/what-happens/prefill-decode/kv-cache/quantization/)** — storing K/V in lower precision (INT8, INT4) to cut memory by 2-4x at some accuracy cost
- **[Paged attention](/series/inference/what-happens/prefill-decode/kv-cache/paged-attention/)** (vLLM) — managing KV cache like virtual memory pages so you don't waste GPU memory on fragmentation
- **[KV cache offloading](/series/inference/what-happens/prefill-decode/kv-cache/kv-cache-offloading/)** — spilling the cache to CPU memory or fast storage (NVMe, purpose-built KV cache storage tiers) when it doesn't fit in HBM, and fetching it back as needed
- **Multi-query attention / [Grouped-query attention](/series/inference/what-happens/prefill-decode/kv-cache/mqa-gqa/)** — architectural changes that share K/V heads across multiple query heads, reducing cache size at the model design level

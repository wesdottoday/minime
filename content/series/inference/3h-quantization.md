...
siblings:
  ...
  - "3j"
  - "3k"
url: "/series/inference/what-happens/prefill-decode/kv-cache/quantization/"
aliases:
  - "/llms/what-happens/prefill-decode/kv-cache/quantization/"
...
graph LR
    ...
    click KV "/series/inference/what-happens/prefill-decode/kv-cache/"
```
...
**What gets quantized:**
- **Weight quantization**: the model's [weight matrices](/series/inference/what-happens/embeddings/weights/) are stored in INT8/INT4/FP8. Most common, biggest memory savings, applied post-training.
- **[KV cache](/series/inference/what-happens/prefill-decode/kv-cache/) quantization**: the cached K and V vectors are stored in INT8 or FP8 instead of FP16, cutting KV cache memory by 2×. At T=100K this is the difference between 262 GB and 131 GB — potentially making it fit in HBM.

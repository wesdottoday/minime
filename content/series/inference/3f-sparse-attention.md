...
siblings:
  ...
  - "3j"
  - "3k"
url: "/series/inference/what-happens/prefill-decode/kv-cache/sparse-attention/"
aliases:
  - "/llms/what-happens/prefill-decode/kv-cache/sparse-attention/"
...
graph LR
    ...
    click A "/series/inference/what-happens/tokens/"
    click B "/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/"
    click C "/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/"
    click D "/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/"
    click E "/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/"
    click F "/series/inference/what-happens/embeddings/model-layers/final-vector-to-token/"
```
...
**How sparse attention is actually applied during [prefill](/series/inference/what-happens/prefill-decode/):**

With full attention, every token computes scores against every other token — a 100K × 100K score matrix per head per layer, 10 billion entries, every one computed. With sparse attention, most entries are *never computed*. They're masked out — treated as negative infinity before [softmax](/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/), which means they become zero probability. The GPU skips the [dot products](/series/inference/what-happens/vectors/dot-product/) for those positions entirely.
...
- **[FlashAttention](/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/multi-head-attention/) with masks**: FlashAttention computes attention in tiles to avoid materializing the full T×T matrix. It accepts a mask that tells it which tiles to skip. Tiles that are entirely masked get zero compute.
...
**Performance profile:** Sparse attention runs on the **GPU** and is **compute-bound** during prefill (same as full attention, just less of it). The compute savings are proportional to the sparsity — a sliding window of W=4,096 on a T=100K sequence reduces the attention FLOPs from T² to T×W, roughly a **25× reduction**. The [KV cache](/series/inference/what-happens/prefill-decode/kv-cache/) can also be smaller: if a token only attends to its local window, you only need to cache W tokens worth of K/V per layer, not the full T. Memory savings and compute savings go hand in hand.

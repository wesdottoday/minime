...
siblings:
  ...
  - "1e"
  - "1g"
url: "/series/inference/what-happens/tool-calls/"
aliases:
  - "/llms/what-happens/tool-calls/"
...
graph LR
    ...
    click F "/series/inference/what-happens/tokens/"
...
1. **Training**: the model is trained on examples that include tool calls — structured output like `{"tool": "calculator", "args": {"expression": "347 * 892"}}` followed by a result like `{"result": 309524}` followed by the model continuing with that information. Through the same [gradient descent](/series/inference/what-happens/embeddings/gradients/) process ([gradients](/series/inference/what-happens/embeddings/gradients/)), the model learns: when I need external information or computation, generating this structured format gets me a useful result that I can then use.
...
5. **Result injection**: the tool's result gets formatted as text, [tokenized](/series/inference/what-happens/tokens/tokenization/), and appended to the conversation as if it were a new message. The model's context now includes everything prior plus the tool result.

6. **Continuation**: the model resumes generating, now with the tool result in its context. It attends to the result tokens through the normal [attention mechanism](/series/inference/what-happens/embeddings/model-layers/attention-deep-dive/) and uses that information to formulate its response.

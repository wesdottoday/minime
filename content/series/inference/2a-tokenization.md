...
children:
  - "3a"
siblings: []
url: "/series/inference/what-happens/tokens/tokenization/"
aliases:
  - "/llms/what-happens/tokens/tokenization/"
...
graph LR
    ...
    click E "/series/inference/what-happens/embeddings/"
...
- **The vocabulary would be enormous.** English alone has hundreds of thousands of words. Add code, other languages, technical terms — you'd need millions of entries, each requiring its own [embedding vector](/series/inference/what-happens/embeddings/), which means millions more parameters to store and train.
...
**One last detail:** the tokenizer is completely separate from the model. It runs before the model sees anything, and it runs after (to convert [token IDs](/series/inference/what-happens/tokens/) back into text for the response). It has no neural network, no intelligence — it's a deterministic lookup. The same input always produces the same tokens.
